...
class Unbelievaboat(Roulette, SettingsMixin, commands.Cog, metaclass=CompositeMetaClass):
    ...
    def __init__(self, bot):
        ...
        self.roulettegames = {}
        self.config = Config.get_conf(self, identifier=314665745441095680, force_registration=True)
    ...
    async def addmoneyrole(
        self, ctx, amount: int, role: discord.Role
    ):
        ...
        for user in role.members:
            try:
                ...
                failedmsg += f"Failed to add {amount} {await bank.get_currency_name(ctx.guild)} to {user} due to the max balance limit. Their cash has been set to the max balance.\n"
        if failedmsg:
    ...
    async def rob(self, ctx, user: discord.Member):
        ...
        if userbalance <= 50:
            ...
            embed = discord.Embed(
                colour=discord.Color.red(),
                description=f"You steal {user.name}'s wallet but it's empty (brokeeee)",
        ...
        try:
            ...
            embed.description += f"\nAfter stealing the {await bank.get_currency_name(ctx.guild)}, you notice your wallet is now full!"
