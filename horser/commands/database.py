...
class DatabaseCommands(MixinMeta):
    ...
    async def dbsetgroup_nukedb(self, ctx: commands.Context, confirm: bool):
        ...
        if not confirm:
            return await ctx.send(f"You must confirm this action with `{ctx.clean_prefix}horserdb nukedb True`")
        config = await self.bot.get_shared_api_tokens("postgres")
        if not config:
            return await ctx.send(f"Postgres credentials not set! Use `{ctx.clean_prefix}horserdb postgres` command!")
        if self.db:
            tmp = self.db
            self.db = None
            tmp.pool.terminate()
